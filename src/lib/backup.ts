...
function sanitizeFilename(filename: string): string {
  return filename.replace(/[^\u4e00-\u9fa5a-zA-Z0-9]/g, '');
}

// 工具函数：生成带日期的文件名
function generateDatedFilename(prefix: string, extension: string): string {
...
export class BackupService {
  ...
  static exportAll(): void {
    ...
    const filename = generateDatedFilename('礼簿备份', 'json');
  ...
  static exportEvent(eventId: string, eventName: string): void {
    ...
    const filename = `礼簿_${safeName}_${new Date().toISOString().slice(0, 10).replace(/-/g, '')}.json`;
    downloadFile(json, filename, 'application/json');
  }
  ...
  static exportTemplate(): void {
    ...
    // 示例数据行
    const exampleData = [
      ['张三', 888, '现金', '新婚快乐', '2025-01-18 10:30'],
      ['李四', 1688, '微信', '百年好合', '2025-01-18 11:00'],
      ['王五', 2000, '支付宝', '早生贵子', '2025-01-18 14:20'],
      ['赵六', 500, '其他', '恭喜发财', '2025-01-18 15:00'],
    ];
    ...
    const instructions = [
      ...
      ['   - 姓名：必填，如：张三', ''],
      ['   - 金额：必填，数字格式，如：888', ''],
      ['   - 支付方式：选填，支持：现金/微信/支付宝/其他', ''],
      ['   - 备注：选填，如：新婚快乐', ''],
      ['   - 录入时间：选填，格式：YYYY-MM-DD HH:MM', ''],
      [''],
      ['2. 注意事项：', ''],
      ['   - 第一行表头不要修改', ''],
      ['   - 示例数据行可以删除', ''],
      ...
      [''],
      ['==========', '========================'],
      ['模板生成时间', new Date().toLocaleString('zh-CN')],
      ['==========', '========================'],
    ];
    ...
    for (let R = 0; R < instructions.length; ++R) {
      for (let C = 0; C < 2; ++C) {
        const cellAddress = XLSX.utils.encode_cell({ r: R, c: C });
        if (!instructionSheet[cellAddress]) continue;

        // 标题行加粗
        if (R <= 2 || instructions[R][0] === '模板生成时间') {
          instructionSheet[cellAddress].s = {
            font: { bold: true, name: '宋体' },
            alignment: { horizontal: 'left', vertical: 'center' }
          };
        } else if (instructions[R][0] === '' || instructions[R][0].includes('1.') || instructions[R][0].includes('2.') || instructions[R][0].includes('3.') || instructions[R][0].includes('4.')) {
          instructionSheet[cellAddress].s = {
            font: { bold: instructions[R][0].includes('1.') || instructions[R][0].includes('2.') || instructions[R][0].includes('3.') || instructions[R][0].includes('4.'), name: '宋体' },
            alignment: { horizontal: 'left', vertical: 'center' }
          };
        } else {
          instructionSheet[cellAddress].s = {
    ...
    // ========== 生成文件并下载 ==========
    const filename = `礼簿导入模板_${new Date().toISOString().slice(0, 10).replace(/-/g, '')}.xlsx`;
